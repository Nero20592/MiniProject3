...
public class KS {
	...
	public KS(Set<KSState> KSStates, Set<KSTransition> kSTransitions, Set<KSState> initialKSStates, Set<AP> ap) {
	...
	public String toString() {
		return "KSStates: " + ksStates.toString() + " Transitions: " + ksTransitions.toString();
	}
	...
	public static KS removeUnreachableKSStates(KS ks) {
		...
		while (!queue.isEmpty()) {
			...
			for (KSTransition t : kSTransitions) {
				...
				if (currentKSState.equals(t.getBegin()) && !reachableKSStates.contains(nextKSState)) {
		...
		for (Iterator<KSTransition> iterator2 = kSTransitions.iterator(); iterator2.hasNext();) {
